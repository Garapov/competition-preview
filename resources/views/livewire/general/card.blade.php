<div class="raffle_card {{ $class }}" x-data="{
    ...
    <div class="raffle_card__in-bottom">
        @php
        ...
        <div class="raffle_card__icon raffle_card__icon--placeholder"></div>
            ...
            <div class="raffle_card__icon" @click="$store.favorites.ids().includes({{ $id }}) ? $store.favorites.removeFromFavorites({{ $id }}) : addToFavorites" :class="{'raffle_card__icon--active': $store.favorites.ids().includes({{ $id }})}">
